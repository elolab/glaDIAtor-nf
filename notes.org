...
process MgfToMzxml {
    ...
    output:
    file "*.mzXML" into pseudospectra_ch,pseudospectra_for_comet_ch,pseudospectra_for_xtandem_ch
...
** TODO Building {Pseudo-,}Spectral library  [0/4]
...
process Comet {
    ...
    // so that xinteract 
    file mzxml from pseudospectra_for_comet_ch
    file fastadb_with_decoy from joined_fasta_with_decoys_ch.first()
    output:
...
process XTandem {
    ...
    input:
    file mzxml from pseudospectra_for_xtandem_ch
    file tandem_config from xtandem_config_ch
    file fastadb_with_decoy from joined_fasta_with_decoys_ch.first()
    output:
...
process CombineSearchResults {
    ...
    output:
    file "lib_iprophet_peps.xml" into combined_search_results_ch 
    """
    /opt/tpp/bin/InterProphetParser DECOY=DECOY_ THREADS=${task.cpus} $xtandem_search_results $comet_search_results lib_iprophet_peps.xml
    """
}

if(params.search_engines.size() == 1)
    combined_search_results_ch.

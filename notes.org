...
def deconvolution_method_is_enabled(method, params){
    ...
    def fallback_method = "deepdia";
    if (params.deconvolution_method){
	return method == params.deconvolution_method
    }

    switch (method) {
	    case "dda": return !!params.dda_files;
	    case "custom": return !!params.speclib;
	    default: return method == fallback_method;
    }
}

def deconvolution_method_is_exclusively_enabled(method, params) {
    deconv_methods = [ "dda","custom", "deepdia", "pseudospectra"]
    return deconv_methods.inject(true) { acc, val -> acc && ( deconvolution_method_is_enabled(val, params)  == (val == methods)) }
...
#+begin_src nextflow :noweb-ref nf-vars :tangle no 
if (deconvolution_method_is_enabled("custom", params)){
...
} else if (deconvolution_method_is_enabled("deepdia",params)){
...
#+HEADER: :tangle no 
#+begin_src emacs-lisp :noweb yes :var program-name="singularity" :var locality="remote" :var gladiator-image="gladiator" :var template=container-config-template :var registry=registry 

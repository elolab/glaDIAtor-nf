...
However, in this developers experience, =xtandem= will crash when using another =max-missed-cleavages= then one so you would
...
process  FindMinimumPeptideProbability {
    ...
    """
    /opt/tpp/bin/Mayu.pl -verbose -A $combined_search_results -C $fastadb_with_decoy -E DECOY_ -G $params.protFDR -P protFDR=${params.protFDR}:t -H 51 -I $max_missed_cleavages -M $prefix
...
if (params.deepdia_min_detectability != null){
	...
	"predict_detectability.py --in $deepdia_prefilt_peptide_list --model $model --out $detect"
    }
	...
	value min_detectability from Channel.value(params.deepdia_min_detectability)
	output:
...
process AddDecoysToOpenSwathTransitions {
    ...
    oswdg_args= deconvolution_method_is_enabled("deepdia",params) ? "-min_decoy_fraction 0.0" : ""
    """
...
process OpenSwathWorkflow_legacy {
	memory { 16.GB * (deconvolution_method_is_enabled("deepdia",params) ? 2 : 1 )}
	input:

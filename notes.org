...
#+PROPERTY: header-args:nextflow :tangle "workflow/legacy-gladiator.nf" :comments link
...
NXF_VER=22.10.1 nextflow -c myconfig.nf \
	 -c config/containers/podman.nf \
	 run workflow/gladiator.nf \
	 --fastafiles='fasta/*.fasta' \
	 --diafiles='MZML/*.mzML' 
#+end_src

if you are using singularity, use =config/containers/singularity.nf= in lieu of =config/containers/podman.nf=.
if you have the guix package manager, you can use =config/containers/guix.nf= .
...
(mapcar 'list (mapcar 'file-relative-name (mapcar 'car (org-babel-tangle-collect-blocks))))
#+end_src

#+RESULTS:
| workflow/legacy-gladiator.nf                 |
| gwl-gladiator.scm                            |
| diaumpireconfig.txt                          |
| comet_template.txt                           |
| xtandem-template.xml                         |
| irt.txt                                      |
| config/containers/singularity.nf             |
| config/containers/singularity-local.nf       |
| config/containers/singularity-guix-local.nf  |
| config/containers/docker.nf                  |
| config/containers/docker-local.nf            |
| config/containers/podman.nf                  |
| config/containers/podman-local.nf            |
| config/containers/guix.nf                    |
| workflow/nextflow.config                     |
| containers/guix/manifests/nextflow-trace.scm |
| nextflow.tags                                |
...
To check, one can manually inspect (using the =gladiator-guix= container), whether  =TargetedFileConverter -in yourlibrary -out speclib.tsv= looks proper.
...
If this steps suggests disabling realignment you can use the =--no_realignment=true= flag
...
<<nf-param-tric_target_fdr>>
#+VINDEX: params.tric_target_fdr
<<nf-param-no_realignment>>
#+VINDEX: params.no_realignment
...
// Set this to true if feature_alignment suggests removing --realign_method 
params.no_realignment = false
// extra parameters to pass to feature_alignment.py
params.feature_alignment_args = ""
#+end_src
...
:PROPERTIES:
:HEADER-ARGS:nextflow: :mkdirp t :tangle (format  "config/containers/%s.config" (replace-regexp-in-string "^config-" "" (or (org-element-property :name (org-element-at-point)) ""))) :noweb tangle
:END:
...
process {
    withName: 'pyprophet_.*' {
        containerOptions = '--env HOME="$PWD"'  // otherwise pyprophet_subsample command fails with: KeyError(key) from None KeyError: 'HOME'
    }
}
#+end_src
...
#+begin_src nextflow
process {
    beforeScript = "source <(guix time-machine -C $projectDir/containers/guix/gladiator-guix-channels.scm -- shell --search-paths -m $projectDir/containers/guix/manifests/gladiator.scm -m $projectDir/containers/guix/manifests/nextflow-trace.scm)"

    withName: 'pyprophet_.*' {
        beforeScript = "source <(guix time-machine -C $projectDir/containers/guix/gladiator-guix-channels.scm -- shell --search-paths -m $projectDir/containers/guix/manifests/pyprophet.scm -m $projectDir/containers/guix/manifests/nextflow-trace.scm)"
    }

    withName: pyprophet_legacy {
        beforeScript = "source <(guix time-machine -C $projectDir/containers/guix/pyprophet-legacy-channels.scm -- shell --search-paths -m $projectDir/containers/guix/manifests/pyprophet-legacy.scm -m $projectDir/containers/guix/manifests/nextflow-trace.scm)"
    }

    withName: 'DIAtracer' {
        beforeScript = "source <(guix time-machine -C $projectDir/containers/guix/diatracer-channels.scm -- shell --search-paths -m $projectDir/containers/guix/manifests/diatracer.scm -m $projectDir/containers/guix/manifests/nextflow-trace.scm)"
    }
}
#+end_src
...
#+begin_src nextflow :tangle workflow/nextflow.config :noweb no-export
<<nf-params>>
#+end_src
...
#+begin_src scheme :tangle containers/guix/manifests/nextflow-trace.scm :noweb yes

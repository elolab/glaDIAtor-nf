...
deconv_input_chs = deconvolution_method.*input.findAll({it != null))
if(deconv_input_chs){
    Channel
	.fromPath(params.diafiles)
	.into(
...
deconv_output_chs = deconvolution_method.*output.findAll({it != null))
...
if(libgen_method_is_enabled("dda",params)){
    deconvolution_methods += [out: { dda_files_ch } ]
}
#+end_src
...
#+begin_src nextflow :noewb-ref nf-vars 
} 
#+end_src
...
if(libgen_method_is_enabled("diams2pep",params){
    deconvolution_methods += [output: diaumpire_pseudospectra_ch,
			      input: diams2pep_mgf_mzml]
}
#+end_src

#+begin_src nextflow :noweb-ref nf-vars :tangle yes
if(libgen_method_is_enabled("diams2pep",params){
#+end_src
...
process convert_for_DIAMS2PEP {
    ...
    output:
    tuple val(task.hash), file(mzml) into diams2pep_mgf_mzml, diams2pep_window_mzml, diams2pep_for_pseudo_mzml
    script:
...
#+begin_src nextflow
process DIAMS2PEP_mgf_generation {
    input:
    tuple val(hash), file(mzml) from diams2pep_mgf_mzml
    output:
    tuple val(hash), file("${mzml.baseName}.mgf") into diams2pep_mgf
    
    """
    RAW.mzMZM.parser.pl mgf $mzml
    """
}
#+end_src
...
process DIAMS2PEP_window {
    input:
    tuple val(hash), file(mzml) from  diams2pep_window_mzml
    output:
    tuple val(hash), file("${mzml}.DIA_acquisition_window.txt") into diams2pep_window
    
    """
    DIA_acquisition_window_generator.pl $mzml
    """
}
#+end_src
...
process DIAMS2PEP_generate_pseudo {
    input:
    tuple val(hash), file(mzml), file(mgf), file(acq_window) from diams2pep_for_pseudo_mzml.join(diams2pep_mgf).join(diams2pep_window)
    ...
    """
    DIA_pesudoM2.pl ${mzml.baseName} mgf-output $tolerance
    """
}
#+end_src

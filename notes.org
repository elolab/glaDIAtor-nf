...
(dia_mzml_files_ch, dia_mzml_files_for_pseudospectra_ch) =
  ...
  Channel.fromPath(params.diafiles).take(2).into(2)
...
process MakeSwathWindows {
    input:
    file diafile from dia_mzml_files_ch.first()
    output: 
...
** TODO Building {Pseudo-,}Spectral library  [1/4]
...
#+end_quote
#+begin_src nextflow
// sepctrast will create *.splib, *.spidx, *.pepidx, 
...
process SpectrastCreateSpecLib {
    ...
    env cutoff from minimum_peptide_probability
    output:
    ...
    script:
    prefix = "SpecLib"
    to_run = "opt/tpp/bin/spectrast -cN${prefix} -cIHCD -cf \"Protein! ~ DECOY_\" -cP$cutoff -c_IRR $combined_search_results "
    if (params.use_irt)
...
process CreateOpenswathTransitions {
    ...
    """
    MINWINDOW=`head -n1 -d'	' -f1 $swath_windows`
    MAXWINDOW=`tail -n1 -d'	' -f2 $swath_windows`
...
http://openswath.org/_/downloads/en/latest/pdf/
*** OpenSwathWorkflow
...
(defun gladiator-compile-test ()
  ...
  (let ((default-directory (concat (file-remote-p default-directory)
    ...
    (compile "module use /appl/user/modulefiles/ && module load nextflow && salloc nextflow -log `mktemp -u XXX.nextflow.log` run gladiator.nf -resume")))

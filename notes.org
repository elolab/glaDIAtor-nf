...
process feature_alignment
{
    publishDir 'data/dia/'
    input:
    file dscore_csvs from pyprophet_legacy_ch.collect()
    output:
...
echo $RUNLOGDIR
sbatch --output=$RUNLOGDIR/slurm.log --err=$RUNLOGDIR/slurm.log --time=11:59:00 nextflow -log $RUNLOGDIR/.nextflow.log run gladiator.nf -with-trace $RUNLOGDIR/trace.txt -with-report $RUNLOGDIR/report.html -with-dag $RUNLOGDIR/graph.dot -resume
#+end_example

...
process CreateSpectrastIrtFile {
    input:
    file irt_traml from Channel.fromPath(params.irt_traml)
    output:
    file ("irt.txt") into irt_txt_ch
    script:
    """
    TargetedFileConverter -in $irt_traml -out_type tsv -out /dev/stdout |"""
    + '''  awk '
    <<irttsv-to-spectrasttsv>>' > irt.txt
    ''' 
}
#+end_src
***** Running Spectrast
...
:PROPERTIES:
:HEADER-ARGS:nextflow: :mkdirp t :tangle (format  "config/%s.nf" (replace-regexp-in-string "^config-" "" (org-element-property :name (org-element-at-point)))) :noweb tangle
:END:

...
** Deconvolution of DIA 
There are various way to deconvolute DIA data 
for generating the spectral library for downstream analysis.
Here we choose between them.


#+begin_src nextflow :noweb-ref nf-params :tangle no
// one of "dda","deepdia","pseudospectra"
// will default to "dda" if ddafiles are supplied
// othewise to "deepdia"
params.deconvolution_method = null
#+end_src


#+begin_src nextflow :noweb-ref nf-vars :tangle no 
// TODO: raise an error if params.deconvolution_method  is not a supported method
#+end_src


#+begin_src nextflow :noweb-ref nf-function-definitions :tangle no 
def deconvolution_method_is_enabled(method, params){
    if (params.deconvolution_method){
	return method == params.deconvolution_method
    }
    if (param.ddafiles){
	return method == "dda"
    }
    return method == "deepdia"
}
#+end_src
...
This section covers both Diaumpire pseudo

#+begin_src nextflow :noweb-ref nf-vars :noweb strip-export
<<nf-pagebreak>> 
if(deconvolution_method_is_enabled("dda",params) || deconvolution_method_is_enabled("pseudospectra",params))
    {
#+end_src
...
dda_assisted_p = !!params.ddafiles
if (dda_assisted_p) {
    Channel.fromPath(params.ddafiles).set{dda_files_ch}
    Channel.empty().set{dia_mzml_files_for_pseudospectra_ch}
} else {
    Channel.fromPath(params.diafiles).set{dia_mzml_files_for_pseudospectra_ch}
}
...
if(deconvolution_method_is_enabled("dda",params)){
    dda_files_ch.multiMap { it ->
	spectrast: comet: xtandem: it }.set{maybespectra_ch}
} else if (deconvolution_method_is_enabled("pseudospectra",params)){
    pseudospectra_ch
...
process CreateOpenswathTransitions {
    ...
    output:
    file outputfile into openswath_transitions_from_pseudo_spectra
...
openswath_transitions_from_pseudo_spectra
    .set{openswath_transitions_ch}
...
#+begin_src nextflow :noweb-ref nf-vars :noweb strip-export
<<nf-pagebreak>>
if(deconvolution_method_is_enabled("deepdia",params))  { 
#+end_src

#+begin_src nextflow :noweb-ref nf-params :tangle no
params._deepdia_url = "https://github.com/lmsac/DeepDIA/blob/c5ad2aa50218fcdfd1d441714702e605fdb00bb3"
// float or null
// if null, do not use minimum detectability filtering
// if a float, filter
params.deepdia_min_detectability = null
params.deepdia_detectability_model = "${params._deepdia_url}/data/detectability/epoch_004.hdf5"
#+end_src
...
params.deepdia_ms2_entries = [
    ["2",
     "${params._deepdia_url}/data/charge2/epoch_035.hdf5",
     ],
    ["3",
     "${params._deepdia_url}/data/charge3/epoch_034.hdf5",
     ]]
#+end_src
...
if(params.deepdia_min_detectability != null){
	...
	.cross(deepdia_ms2_models)
	.map(peptides,chargeandmodel ->
	    tuple(chargeandmodel[0],chargeandmodel[1],
		  peptides))
...
if (params.deepdia_min_detectability != null){
	...
	detect="{deepdia_prefilt_peptide_list.baseName}.detectability.csv"
	"""
	predict_detectability --in $deepdia_prefilt_peptide_list --model $model --out $detect
	filter_peptide_by_detectability --peptide $deepdia_prefilt_peptide_list --detect $detect --min_detectability ${params.deepdia_min_detectability} --out ${deepdia_filtered_peptide_list}
	"""
    }
}
...
process DeepDIAPredictCharge {
    input:
    set charge,file(model),file(peptides) from deepdia_ms2_inputs_ch
    output:
    file deepdia_ions
    script:
...
deepdia_peptides = Channel.fromPath(params.deepdia_peptides)
#+end_src
...
process DeepDIAPredictRetention {
    ...
    file deepdia_peptides
    output:
    file predicted_rt
    script:
    predicted_rt="prediction.irt.csv"
    """
    predict_rt --in $deepdia_peptides --model $deepdia_irt_model --out $predicted_rt
    """
}
#+end_src
...
process DeepDIAPredictionsToLibrary {
    ...
    """
    build_assays_from_prediction --peptides ${deepdia_peptides} --rt ${predicted_rt} --ions ${ions} --out prediction.assay.pickle
    convert_assays_to_Spectronaut_library.py --in prediction.assay.pickle --out ${deepdia_speclib}
    """
}
#+end_src

#+begin_src nextflow :noweb-ref nf-vars
   deepdia_speclib.set{openswath_transitions_ch}
...
If =pyprophet merge= complains about no decoys being found,

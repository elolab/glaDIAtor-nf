...
* Preprocessing Data
This will not be in the main pipeline
** Picking Peaks
#+begin_src sh 
mkdir -p MZXML-pwiz
...
process XinteractComet {
    ...
    // 
    input:
    file pepxmls from comet_pepxml_ch.collect()
    ...
    file fastadb_with_decoy from joined_fasta_with_decoys_ch.first()
    file mzxmls from  xinteract_comet_mzxml_ch.collect()
    output: 
...
#+begin_src nextflow :noweb-ref nf-params :tangle no
params.precursor_mass_tolerance=20 // ppm 
params.fragment_mass_tolerance=0.3  // Dalton
...
process XinteractXTandem {
    memory '16 GB'
    input:
    file pepxmls from xtandem_pepxml_ch.collect()
    // the filename of needed fastdadb was defined in cometcfg
    // and stored in pepxml in the comet-ms step
    // -a suppplies the absulute path to the data directory where the mzxmls
    // rather than reading wherer the mfrom the xmls
    ...
    // nextflow to look outside the cwd.
    file fastadb_with_decoy from joined_fasta_with_decoys_ch.first()
    file mzxmls from  xinteract_xtandem_mzxml_ch.collect()
    output: 
...
process CombineSearchResults {
    publishDir 'data/speclib'
    when:
    params.search_engines.size() > 1
    input:
    file xtandem_search_results from xtandem_search_results_ch
    file comet_search_results from comet_search_results_ch
    output:
    file "lib_iprophet.peps.xml" into both_combined_search_results_ch
    """
...
if(params.search_engines.size()) > 1 {
    both_combined_search_results_ch.tap{combined_search_results_ch}
} else if (params.search_engines.contains("comet")) {
    comet_search_results_ch.tap{combined_search_results_ch}
    
} else if (params.search_engines.contains("xtandem")) {
    xtandem_search_results_ch.tap{combined_search_results_ch}
}
#+end_src
...
process SpectrastCreateSpecLib {
    ...
    file combined_search_results from combined_search_results_ch.first()
    file fastadb_with_decoy from joined_fasta_with_decoys_ch.first()
    file spectra from maybespectra_ch.spectrast.collect()
...
process CreateOpenswathTransitions {
    ...
    script:
    outputfile="SpecLib_cons_decoys_openswath.tsv"
    ...
    OpenSwathDecoyGenerator -in SpecLib_cons_openswath.tsv -out SpecLib_cons_decoys_openswath.tsv -method reverse
    """
}
#+end_src
...
process pyprophet_legacy {
    publishDir 'data/pyprophet/' 
    input:
    file dia_tsv from openswath_tsv_ch
    output:
    file dscore_csv into pypropet_legacy_ch
    script:
    seed="928418756933579397"
    
    dscore_csv="${dia_tsv.baseName}_with_dscore.csv"
    
    "/opt/pyprophet/bin/pyprophet --random_seed=${seed} --delim=tab --export.mayu ${dia_tsv} --ignore.invalid_score_columns"
}
#+end_src
...
process feature_alignment
{
    publishDir 'data/dia/'
    input:
    file dscore_csvs from pypropet_legacy_ch.collect()
    output:
...
process swath2stats {
	...
	peptideoutputfile="$peptidematrix",
	proteinoutputfile="$proteinmatrix",
	decoyprefix="DECOY_")
...
#+begin_src nextflow :tangle nextflow.config :noweb yes
singularity.autoMounts = true
process.container="file://../../gladiator/gladiator-images/gladiator-pyprophet-legacy.simg"

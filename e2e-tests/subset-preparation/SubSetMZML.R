...
pxml.s <- lapply(pxml, function(x) {
    return(x[x[, "peptide"] %in% peptide_wl, ])
})


pick_best_spectra <- function(x) {
    x.s <- split(x, x[, "peptide"])
    x.s <- lapply(x.s, function(s) {
        s.rat <- as.numeric(s[, "num_matched_ions"]) / as.numeric(s[, "tot_num_ions"])
        s <- s[order(-s.rat), ]
        return(s[1, ])
    })
...
scans <- lapply(pxml.s, function(x) {
    return(as.numeric(gsub("^.+ scan=", "", x[, "spectrumNativeID"])))
})
...
subset_MZ <- function(x, add_dummy, u_ofs, d_ofs) {
    ...
    writeMSData(
        object = pks, file = out_file, header = hdr,
